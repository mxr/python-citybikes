...
async def test_networks(client):
    ...
    assert networks == [
        Network.from_dict(n) for n in mockresponses.networks["networks"]
    ]
...
async def test_networks_near(client):
    ...
    assert [n.id for n, dist in client.networks.near(30.0, 1.0)] == [
        "bar",
        "baz",
        "foo",
    ]
    assert [n.id for n, dist in client.networks.near(100.0, 100.0)] == [
        "foo",
        "bar",
        "baz",
    ]
