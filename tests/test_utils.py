...
class TestDistanceSort:
    ...
    def test_eq(self):
        pairs = dist_sort([0.0, 0.0], self.points, lambda p: (p[0], p[1]))
        sorted_points = [latlng for latlng, dist in pairs]
        assert self.points == sorted_points

    def test_sort(self):
        pairs = dist_sort([10.0, 10.0], self.points, lambda p: (p[0], p[1]))
        sorted_points = [latlng for latlng, dist in pairs]
